...
export default function DuotonePage() {
  ...
  const [color1, setColor1] = React.useState("#1e3a8a");
  const [color2, setColor2] = React.useState("#f59e0b");
  ...
  const process = useBatchProcess({
    ...
    task: async (item) => {
      ...
      for (let i = 0; i < data.length; i += 4) {
        ...
        data[i] = Math.round(c1[0] * (1 - lum) + c2[0] * lum);
        data[i + 1] = Math.round(c1[1] * (1 - lum) + c2[1] * lum);
        data[i + 2] = Math.round(c1[2] * (1 - lum) + c2[2] * lum);
      }
      ...
      const name = withExtension(`${getBaseName(item.name)}-duotone`, format);
        ...
        <div className="mx-auto max-w-5xl space-y-6 p-8">
          ...
          <div className="bg-card space-y-4 rounded-lg border p-5">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-sm font-medium">双色调选项</h2>
            </div>
