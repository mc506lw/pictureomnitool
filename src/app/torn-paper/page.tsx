...
export default function TornPaperPage() {
  ...
  const [roughness, setRoughness] = React.useState(20);
  ...
  const process = useBatchProcess({
    ...
    task: async (item) => {
      ...
      ctx.beginPath();
      const step = 8;
      const amplitude = roughness;
      ctx.moveTo(0, canvas.height);
      for (let x = 0; x <= canvas.width; x += step) {
        const noise = (Math.random() - 0.5) * amplitude;
        ctx.lineTo(x, canvas.height - Math.abs(noise) - 2);
      }
      ctx.lineTo(canvas.width, 0);
      for (let x = canvas.width; x >= 0; x -= step) {
        const noise = (Math.random() - 0.5) * amplitude;
        ctx.lineTo(x, Math.abs(noise) + 2);
      }
      ctx.closePath();
      ctx.clip();
      ...
      const name = withExtension(
        `${getBaseName(item.name)}-torn-paper`,
        format
      );
        ...
        <div className="mx-auto max-w-5xl space-y-6 p-8">
          ...
          <div className="bg-card space-y-4 rounded-lg border p-5">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-sm font-medium">撕纸选项</h2>
            </div>
