...
import { Waves, Play, Square as StopSquare, Settings2 } from "lucide-react";
...
export default function RipplePage() {
  ...
  const process = useBatchProcess({
    ...
    task: async (item) => {
      ...
      for (let y = 0; y < h; y++) {
        for (let x = 0; x < w; x++) {
          ...
          if (dist < radius) {
            ...
            const shift = Math.cos(factor * Math.PI * 2) * strength * factor;
            const srcX = Math.round(x + Math.cos(angle) * shift);
            const srcY = Math.round(y + Math.sin(angle) * shift);
            const clampedX = Math.min(w - 1, Math.max(0, srcX));
            const clampedY = Math.min(h - 1, Math.max(0, srcY));
      ...
      const name = withExtension(`${getBaseName(item.name)}-ripple`, format);
        ...
        <div className="mx-auto max-w-5xl space-y-6 p-8">
          ...
          <div className="bg-card space-y-4 rounded-lg border p-5">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-sm font-medium">水波选项</h2>
            </div>
