...
import { Zap, Play, Square as StopSquare, Settings2 } from "lucide-react";
...
export default function GhostTrailPage() {
  ...
  const [shift, setShift] = React.useState(16);
  const [fade, setFade] = React.useState(60);
  ...
  const process = useBatchProcess({
    ...
    task: async (item) => {
      ...
      const stepAlpha = 1 / steps;
      for (let i = 0; i < steps; i++) {
        ctx.globalAlpha = stepAlpha * (fade / 100);
        ctx.drawImage(
          src,
          shift * i,
          0,
      ...
      const name = withExtension(`${getBaseName(item.name)}-ghost`, format);
        ...
        <div className="mx-auto max-w-5xl space-y-6 p-8">
          ...
          <div className="bg-card space-y-4 rounded-lg border p-5">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-sm font-medium">拖尾选项</h2>
            </div>
