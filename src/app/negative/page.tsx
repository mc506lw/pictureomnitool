  ...
  const process = useBatchProcess({
    ...
    task: async (item) => {
      ...
      for (let i = 0; i < len; i += 4) {
        data[i] = 255 - data[i] + brightness;
        data[i + 1] = 255 - data[i + 1] + brightness;
        data[i + 2] = 255 - data[i + 2] + brightness;
      }
      ...
      const name = withExtension(`${getBaseName(item.name)}-negative`, format);
        ...
        <div className="mx-auto max-w-5xl space-y-6 p-8">
          ...
          <div className="bg-card space-y-4 rounded-lg border p-5">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-sm font-medium">负片选项</h2>
            </div>
