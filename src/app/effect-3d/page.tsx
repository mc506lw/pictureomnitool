...
export default function Effect3DPage() {
  ...
  const process = useBatchProcess({
    ...
    task: async (item) => {
      ...
      const shearX = Math.tan(rad) * depth;
      const canvas = document.createElement("canvas");
      canvas.width = Math.max(1, Math.round(w + Math.abs(shearX)));
      canvas.height = Math.max(1, Math.round(h + depth));
      const ctx = canvas.getContext("2d")!;
      ctx.fillStyle = "#000000";
      ctx.fillRect(0, 0, canvas.width, canvas.height);
      ctx.translate(canvas.width / 2, canvas.height - depth);
      ctx.transform(cos, sin, -sin, cos, -w / 2, -h);
      ...
      const name = withExtension(`${getBaseName(item.name)}-3d`, format);
        ...
        <div className="mx-auto max-w-5xl space-y-6 p-8">
          ...
          <div className="bg-card space-y-4 rounded-lg border p-5">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-sm font-medium">3D 选项</h2>
            </div>
