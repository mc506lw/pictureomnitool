...
export default function ReflectionPage() {
  ...
  const process = useBatchProcess({
    ...
    task: async (item) => {
      ...
      const reflHeight = Math.round((src.height * ratio) / 100);
      const canvas = document.createElement("canvas");
      canvas.width = src.width;
      canvas.height = src.height + reflHeight;
      ...
      ctx.save();
      ctx.translate(0, src.height * 2 + reflHeight);
      ctx.scale(1, -1);
      ctx.drawImage(src, 0, 0);
      ctx.restore();
      ...
      ctx.fillRect(0, src.height, canvas.width, reflHeight);
      ...
      const name = withExtension(
        `${getBaseName(item.name)}-reflection`,
        format
      );
        ...
        <div className="mx-auto max-w-5xl space-y-6 p-8">
          ...
          <div className="bg-card space-y-4 rounded-lg border p-5">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-sm font-medium">倒影选项</h2>
            </div>
